...
public class DeptosEditorUi extends Composite {
    ...
    private final CellList<Empleado> cellListEmpleados;
...
//        flexEditorGeneral.getColumnFormatter().setWidth(0, "50px;");
        ...
        cellListEmpleados = new CellList<>(new EmpleadoCell(), cellListResources, EmpleadosProvider.get().getDataProvider());
...
//        pager.setDisplay(dataGrid);
        ...
        EmpleadosProvider.get().addDataDisplay(cellListEmpleados);
        ...
        EmpleadosProvider.get().addMethodExecutedListener(new EmpleadosProviderMethodExecutedListener());
...
//                    builder.append("<br/>");
    ...
    private class EmpleadosProviderMethodExecutedListener implements EmpleadosProvider.MethodExecutedListener {

        @Override
        public void onMethodExecuted(MethodEvent methodEvent) {
            if (EMethod.FIND_ALL_BY_DEPTO.equals(methodEvent.getMethod())) {
                
                tabEditorMiembros.setText("Miembros (" + EmpleadosProvider.get().getDataProvider().getList().size() + ")");
            }
        }
    ...
    private class EmpleadoCell extends AbstractCell<Empleado> {
        ...
        public void render(Cell.Context context, Empleado value, SafeHtmlBuilder sb) {
            ...
            html = html.replace("APELLIDOS_REEMPLAZO", EmpleadosUI.ellipse(chkStrNull(value.getApellidoPaterno()), 60) 
            ...
            html = html.replace("DEPTO_NAME_REEMPLAZO", EmpleadosUI.ellipse(chkStrNull(deptoNameStr), 80));
